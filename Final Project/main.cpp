...
void initialise()
{
    ...
    g_current_scene->m_state.bgm = Mix_LoadMUS("Ancient Mystery Waltz Presto.mp3");
    Mix_PlayMusic(g_current_scene->m_state.bgm,-1);

    Mix_VolumeMusic(MIX_MAX_VOLUME / 4.0f);
    g_current_scene->m_state.hit_sfx = Mix_LoadWAV("punch.wav", 0.1);
...
void process_input()
{
    if (g_current_scene != g_menu)
    {
        ...
        {
            Mix_PlayChannel(-1, g_current_scene->m_state.hit_sfx, 0);
            if (glm::distance(g_current_scene->m_state.player->get_position(), g_current_scene->m_state.enemies[0].get_position()) < 2) {
                g_current_scene->m_state.enemies[0].deactivate();
                g_current_scene->m_state.enemies[0].m_is_defeated = true;
...
void update()
{
    ...
    if (!end_game)
    {
        ...
        {
            if (lives == 0) {
                g_current_scene->m_state.player->lose = true;
                //end_game = true;
            }
            ...
            /*if (g_current_scene->m_state.player->get_position().y > LEVEL1_TOP_EDGE) {
                g_view_matrix = glm::translate(g_view_matrix, glm::vec3(-5, -g_current_scene->m_state.player->get_position().y, 0));
            }
            else {
                g_view_matrix = glm::translate(g_view_matrix, glm::vec3(-5, 3.75, 0));
            }*/
            if (g_current_scene->m_state.player->die) {
                g_current_scene->m_state.player->die = false;
                lives--;
            }
...
void render()
{
    ...
    /*if (g_current_scene!=g_menu)
    {
        std::string lives = std::to_string(g_current_scene->m_state.player->lives);
        Utility::draw_text(&g_shader_program, font_id, lives, 1, 0, glm::vec3(-4.0, 3.0, 0.0));
    }*/
    GLuint text_id = Utility::load_texture("font1.png");
    if (g_current_scene != g_menu)
    {
        std::string liveschar = "lives:" + std::to_string(lives);
        Utility::draw_text(&g_shader_program, text_id, liveschar, 0.5, 0, glm::vec3(g_current_scene->m_state.player->get_position().x-4, 
                                                                                    g_current_scene->m_state.player->get_position().y+3, 0));
    }

    if (g_current_scene != g_menu && g_current_scene->m_state.player->lose) {
        Utility::draw_text(&g_shader_program, text_id, "YOU LOSE", 0.5, 0, g_current_scene->m_state.player->get_position());
        end_game = true;
    }
    if (g_current_scene != g_menu && g_current_scene->m_state.player->win) {
        Utility::draw_text(&g_shader_program, text_id, "YOU WIN", 0.5, 0, g_current_scene->m_state.player->get_position());
        end_game = true;
    }
